...
def kallisto_quant(genome,reads):
    kallisto_counts=subprocess.check_output('kallisto quant -i {} -o kallisto_quant_SRR4031333 --single -l 51 -s 10 {}'.format(genome,reads), shell=True)  # specify you want to capture STDOUT
   # stdout = rtn.decode('utf-8')
   # lines = stdout.splitlines()
